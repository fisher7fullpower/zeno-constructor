...
7. **AI:** Groq (LLM) + Decor8.ai (рендеры интерьеров, 51 стиль, 29 типов комнат) + Replicate (Extreme restyle, Grounding DINO — только constructor.html).
...
|------|--------|-----------|
| `/etc/proxy-constructor.env` | proxy-constructor | REPLICATE_TOKEN, DECOR8_TOKEN, GROQ_API_KEY, RESEND_KEY, FROM_EMAIL, JWT_SECRET |
...
export REPLICATE_TOKEN=... DECOR8_TOKEN=... GROQ_API_KEY=... RESEND_KEY=... JWT_SECRET=...
...
Пользователь → nginx (443 SSL) → proxy.py (Flask API)
                                       ...
                                       ├── Decor8.ai (interior design renders)
                                       ├── Groq API (LLM chat)
                                       ├── Resend (email)
                                       └── JSON files (data/)
...
| POST | `/api/decor8/<endpoint>` | — | Прокси к Decor8.ai (12 эндпоинтов) |
| POST | `/api/decor8/advisor` | — | AI-дизайн консультант (Groq LLM) |
| POST | `/api/renders/check` | — | Проверить остаток рендеров |
| POST | `/api/renders/use` | — | Использовать рендер |
